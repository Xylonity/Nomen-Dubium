...
import dev.xylonity.nomendubium.client.entity.model.body.ChimeraBodyModel;
...
import dev.xylonity.nomendubium.client.model.ChimeraModelLayers;
...
public final class ChimeraRenderer extends EntityRenderer<ChimeraEntity, ChimeraRenderState> {
    ...
    public ChimeraRenderer(EntityRendererProvider.Context context) {
        ...
        bodies.put(
            ChimeraBodyVariant.HULKING,
            body(new HulkingBodyModel(context.bakeLayer(ChimeraModelLayers.HULKING_BODY)), "hulkingbody")
        );
        bodies.put(
            ChimeraBodyVariant.SHELLED,
            body(new ShelledBodyModel(context.bakeLayer(ChimeraModelLayers.SHELLED_BODY)), "shelledbody")
        );
        bodies.put(
            ChimeraBodyVariant.AVIAN,
            body(new AvianBodyModel(context.bakeLayer(ChimeraModelLayers.AVIAN_BODY)), "avianbody")
        );
        bodies.put(
            ChimeraBodyVariant.LANKY,
            body(new LankyBodyModel(context.bakeLayer(ChimeraModelLayers.LANKY_BODY)), "lankybody")
        );
        bodies.put(
            ChimeraBodyVariant.PUFFY,
            body(new PuffyBodyModel(context.bakeLayer(ChimeraModelLayers.PUFFY_BODY)), "puffybody")
        );
        heads.put(
            ChimeraHeadVariant.CRUNCHING,
            part(new CrunchingHeadModel(context.bakeLayer(ChimeraModelLayers.CRUNCHING_HEAD)), "crunchinghead")
        );
        heads.put(
            ChimeraHeadVariant.SHIELDED,
            part(new ShieldedHeadModel(context.bakeLayer(ChimeraModelLayers.SHIELDED_HEAD)), "shieldedhead")
        );
        heads.put(
            ChimeraHeadVariant.SNARLED,
            part(new SnarledHeadModel(context.bakeLayer(ChimeraModelLayers.SNARLED_HEAD)), "snarledhead")
        );
        heads.put(
            ChimeraHeadVariant.BEAKED,
            part(new BeakedHeadModel(context.bakeLayer(ChimeraModelLayers.BEAKED_HEAD)), "beakedhead")
        );
        heads.put(
            ChimeraHeadVariant.SNORTING,
            part(new SnortingHeadModel(context.bakeLayer(ChimeraModelLayers.SNORTING_HEAD)), "snortinghead")
        );
        tails.put(
            ChimeraTailVariant.SPIKED,
            part(new SpikedTailModel(context.bakeLayer(ChimeraModelLayers.SPIKED_TAIL)), "spikedtail")
        );
        tails.put(
            ChimeraTailVariant.STUBBY,
            part(new StubbyTailModel(context.bakeLayer(ChimeraModelLayers.STUBBY_TAIL)), "stubbytail")
        );
        tails.put(
            ChimeraTailVariant.CLUBBED,
            part(new ClubbedTailModel(context.bakeLayer(ChimeraModelLayers.CLUBBED_TAIL)), "clubbedtail")
        );
        tails.put(
            ChimeraTailVariant.FAN,
            part(new FanTailModel(context.bakeLayer(ChimeraModelLayers.FAN_TAIL)), "fantail")
        );
        tails.put(
            ChimeraTailVariant.SPEARED,
            part(new SpearedTailModel(context.bakeLayer(ChimeraModelLayers.SPEARED_TAIL)), "spearedtail")
        );
        backs.put(
            ChimeraBackVariant.BONEY_PLATES,
            part(new BoneyPlatesModel(context.bakeLayer(ChimeraModelLayers.BONEY_PLATES)), "boneyplates")
        );
        backs.put(
            ChimeraBackVariant.DORSAL_SCALES,
            part(new DorsalScalesModel(context.bakeLayer(ChimeraModelLayers.DORSAL_SCALES)), "dorsalscales")
        );
        backs.put(
            ChimeraBackVariant.SPIKES,
            part(new SpikesModel(context.bakeLayer(ChimeraModelLayers.SPIKES)), "spikes")
        );
        backs.put(
            ChimeraBackVariant.SPINE_SAIL,
            part(new SpineSailModel(context.bakeLayer(ChimeraModelLayers.SPINE_SAIL)), "spinesail")
        );
        backs.put(
            ChimeraBackVariant.THORNS,
            part(new ThornsModel(context.bakeLayer(ChimeraModelLayers.THORNS)), "thorns")
        );
    }
    ...
    public void submit(ChimeraRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        ...
        RenderedBody body = bodies.get(state.body);
    ...
    private static RenderedPart part(EntityModel<ChimeraRenderState> model, String texture) {
        Identifier baseTexture = NomenDubium.of("textures/entity/chimera/" + texture + ".png");
        Map<ChimeraPaletteVariant, SpriteId> paletteSprites = new EnumMap<>(ChimeraPaletteVariant.class);
        for (ChimeraPaletteVariant palette : ChimeraPaletteVariant.values()) {
    ...
    private static SpriteId sprite(String texture, String palette) {
        return new SpriteId(
            ...
            NomenDubium.of("entity/chimera/" + texture + "_" + palette)
        );
    }

    private void submitPart(
        RenderedPart part,
        ChimeraRenderState state,
        PoseStack poseStack,
    ...
    private void submitAttachedPart(
        RenderedPart part,
        ChimeraBodyModel body,
    ...
    private record RenderedBody(ChimeraBodyModel model, RenderedPart part) {
    }
